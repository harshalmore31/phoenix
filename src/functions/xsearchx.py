...
os = lazy_import("os")
List, Dict, Optional = lazy_import("typing").List, lazy_import("typing").Dict, lazy_import("typing").Optional
...
class WebSearch:
                ...
                async with session.get(url, params=params) as response:
                    if response.status == 200:
                        ...
                        return [
                            ...
                            if "link" in item and not any(ext in item["link"].lower() for ext in [".pdf", ".doc", ".docx"])
                        ][:10]
                    return []
    ...
    def _preprocess_html(self, raw_html: str) -> str:
        ...
        cleaned_html = re.sub(r'<img[^>]+src=".*?(track|pixel|beacon).*?"[^>]*>', '', cleaned_html, flags=re.IGNORECASE)
        ...
        return content_match.group(1) if content_match else cleaned_html
        ...
        try:
            with sync_playwright() as p:
                ...
                try:
                    ...
                    for element in soup(['nav', 'footer', 'header', 'aside', 'form', 'iframe']):
                        element.decompose()
                    ...
                    clean_text = self.html_converter.handle(main_content) if main_content else ""
                    clean_text = " ".join(clean_text.split())  # Normalize whitespace
                    
                    return {
                        "url": url,
                        "title": soup.title.string.strip() if soup.title else "No title",
                        "content": clean_text[:3000] + "..." if len(clean_text) > 3000 else clean_text
                    }
            ...
            console.print(f"[yellow]Content extraction warning: {url} - {str(e)}[/yellow]")
            return None
    ...
    def format_results(self, query: str, results: List[Dict]) -> str:
        ...
        for i, result in enumerate(results, 1):
            content_preview = result.get('content') or result.get('snippet') or "Content unavailable"
            formatted.extend([
                f"{i}. {result['title']}",
                f"   URL: {result['url']}",
    ...
    async def search(self, query: str) -> str:
        ...
        content_map = {item['url']: item for item in extracted_data if item}
        ...
        for original in search_data:
            merged_results.append({
                **original,
                "content": content_map.get(original['url'], {}).get('content', '')
            })
        ...
        filename = os.path.join(self.outputs_dir, f"search_{timestamp}.txt")
