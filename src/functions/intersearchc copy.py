                ...
                async with session.get(url, params=params) as response:
                    if response.status == 200:
                        ...
                        return results[:10]  # Ensure we only take top 10
                         ...
                         logging.error(f"Error fetching search results: {error_message}")
                         return []
        ...
        try:
            with sync_playwright() as p:
                ...
                try:
                    ...
                    clean_text = str(main_content) # Fallback to raw HTML if conversion fails
                    logging.warning(f"html2text conversion failed for {url}: {str(e)}. Falling back to raw HTML.")
...
"""
        ...
        try:
            ...
            return response
             ...
             console.print(f"[red]Error during summarization: {str(e)}[/red]")
    ...
    async def search(self, query: str) -> str:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S") # New Line
        ...
        output_filename = os.path.join(self.outputs_dir, f"search_results_{timestamp}.json") # Modified Line
        ...
        console.print(f"[bold]Search Time:[/bold] {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}") # New Line
        console.print(f"[bold]Query:[/bold] {query}") # New Line
