...
custom_ppn_path = r"assets/wake_word_detect/Phoenix_en_windows_v3_0_0.ppn"
    ...
    while True:
        if detect_wake_word(custom_ppn_path, access_key):
            ...
            if isinstance(ai_response, dict) and 'output' in ai_response:
                response_content = ai_response['output']
                console.print(Panel(response_content, style="bold green", box=SIMPLE))
                speak(response_content)
            else:
                console.print("Unexpected response structure:", ai_response)
